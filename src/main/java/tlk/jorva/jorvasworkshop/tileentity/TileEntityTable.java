...
import tlk.jorva.jorvasworkshop.gui.menu.GuiMenu;
import tlk.jorva.jorvasworkshop.gui.menu.GuiMenuItem;
...
import tlk.jorva.jorvasworkshop.network.LengthCount;
import tlk.jorva.jorvasworkshop.network.PacketHandler;
import tlk.jorva.jorvasworkshop.network.PacketId;
import tlk.jorva.jorvasworkshop.network.data.DataType;
import tlk.jorva.jorvasworkshop.page.Page;
import tlk.jorva.jorvasworkshop.page.PageMain;
import tlk.jorva.jorvasworkshop.page.PageTransfer;
import tlk.jorva.jorvasworkshop.page.PageUpgrades;
import tlk.jorva.jorvasworkshop.page.setting.Setting;
import tlk.jorva.jorvasworkshop.page.setting.Side;
import tlk.jorva.jorvasworkshop.page.setting.Transfer;
...
public class TileEntityTable extends TileEntity implements IInventory, ISidedInventory, IFluidHandler, ITickable, /*TESLA*/ ITeslaHolder, ITeslaConsumer {
    ...
    private void transfer(Setting setting, Side side, Transfer transfer, int transferSize) {
        if (transfer.isEnabled() && transfer.isAuto()) {
            EnumFacing direction = EnumFacing.values()[BlockWorkshopTable.getSideFromSideAndMetaReversed(side.getDirection().ordinal(), getBlockMetadata())];
            TileEntity te = worldObj.getTileEntity(new BlockPos(pos.getX() + direction.getFrontOffsetX(), pos.getY() + direction.getFrontOffsetY(), pos.getX() + direction.getFrontOffsetZ()));
            if (te instanceof IInventory) {
                IInventory inventory = null;
                if (te instanceof TileEntityChest) {
//                    inventory = Blocks.CHEST.func_149951_m(te.getWorld(), te.getPos().getX(), te.getPos().getY(), te.getPos().getX());
                	if (inventory == null) {
                        return;
                    }
                }else{
                ...
                if (inventory instanceof ISidedInventory) {
                    slots2 = ((TileEntityTable)inventory).getSlotsForFace(directionReversed);
                }else{
    ...
    private int getTransferSide(EnumFacing side) {
        return BlockWorkshopTable.getSideFromSideAndMeta(side.ordinal(), getBlockMetadata());
    }
