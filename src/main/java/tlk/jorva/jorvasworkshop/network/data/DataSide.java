...
import tlk.jorva.jorvasworkshop.page.setting.*;
...
public abstract class DataSide extends DataBase {
    ...
    protected tlk.jorva.jorvasworkshop.page.setting.Transfer getTransfer(tlk.jorva.jorvasworkshop.tileentity.TileEntityTable table, int id) {
        int settingId = id % SETTINGS;
        id /= SETTINGS;
        int sideId = id % SIDES;
        id /= SIDES;
        int modeId = id;
        tlk.jorva.jorvasworkshop.page.setting.Side side = table.getTransferPage().getSettings().get(settingId).getSides().get(sideId);
        if (modeId == 0) {
    ...
    public static int getId(tlk.jorva.jorvasworkshop.page.setting.Setting setting, tlk.jorva.jorvasworkshop.page.setting.Side side, tlk.jorva.jorvasworkshop.page.setting.Transfer transfer) {
    ...
    public static class Enabled extends DataSide {
        @Override
        public void save(TileEntityTable table, tlk.jorva.jorvasworkshop.network.DataWriter dw, int id) {
